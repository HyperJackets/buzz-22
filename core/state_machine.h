...
// Push changes to current branch as sean, and see changes online at
...
private:
  ...
  // variable, and f means that it's a float (decimal), and b means that it's a
  // boolean.
  double m_fVelocity, m_fAccel, m_fBatteryTemp, m_fMotorTemp, m_fCurrentDist;
  bool m_bHealth, m_bLocalizationHealth;
  // The following variables denote if the interface is engaged or not. Also
  // make sure that VD and structures know that braking and propulsion can never
  // be simultaneously powered.
  ...
  // constants
  static const int THRESHOLD = 100, EXCEEDED_DIST = 170, TRACK_DIST = 150,
                   BREAK_DIST = 70;
...
public:
  ...
  bool getCommunicationsEngagement(void) const {
    return this->m_bCommunications;
  }
  ...
  void setLocalizationEngagement(bool bLocalization) {
    this->m_bLocalization = bLocalization;
  }
  ...
  void setCommunicationsEngagement(bool bCommunications) {
    this->m_bCommunications = bCommunications;
  }

  // set methods for GUI
  void setIsUnloaded(bool bIsUnloaded) { this->m_bIsUnloaded = bIsUnloaded; }
  void setIsReadyToLaunch(bool bIsReadyToLaunch) {
    this->m_bIsReadyToLaunch = bIsReadyToLaunch;
  }
  ...
  // These are methods that will be used in the test. However when we integrate
  // with hardware the hardware will monitor and calculate the data.
  void setVelocity(double fVelocity) { this->m_fVelocity = fVelocity; }
  void setAccel(double fAccel) { this->m_fAccel = fAccel; }
  void setBatteryTemp(double fBatteryTemp) {
    this->m_fBatteryTemp = fBatteryTemp;
  }
  void setMotorTemp(double fMotorTemp) { this->m_fMotorTemp = fMotorTemp; }
  void setCurrentDist(double fCurrentDist) {
    this->m_fCurrentDist = fCurrentDist;
  }
  void setHealth(bool bHealth) { this->m_bHealth = bHealth; }
  void setLocalizationHealth(bool bLocalizationHealth) {
    this->m_bLocalizationHealth = bLocalizationHealth;
  }
